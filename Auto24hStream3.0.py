...
class StreamerApp:
    def __init__(self, root):
        ...
        self.root.title(self.get_translation('app_title')+'V3.0 By QyJoy')
        ...
        self.themes = {
            ...
            "白色 (White)": {"bg": "#FFFFFF", "fg": "black", "widget_bg": "#F5F5F5", "widget_fg": "black", "button_bg": "#EEEEEE", "button_fg": "black", "disabled_fg": "#B0B0B0", "log_bg": "#FAFAFA", "log_fg": "black", "select_bg": "#CDE8FF", "select_fg": "black"},
    ...
    def setup_language(self):
        ...
        self.translations = {
            "zh_CN": {
                "app_title": "FFmpeg 24/7 Video Streamer GUI (自动循环推流)", "menu_language": "语言 (Language)", "menu_theme": "主题 (Theme)",
                ...
                "start_button": "开始推流", "stop_button": "停止推流", "log_output_label": "日志输出",
            ...
            "en_US": {
                "app_title": "FFmpeg 24/7 Streamer GUI (Auto Loop)", "menu_language": "Language", "menu_theme": "Theme",
                ...
                "start_button": "Start Streaming", "stop_button": "Stop Streaming", "log_output_label": "Log Output",
    ...
    def create_menu(self):
        ...
        # Language Menu - Items added here, cascade label set initially
        self.lang_menu = Menu(self.menubar, tearoff=0)
        self.lang_menu.add_radiobutton(label=self.get_translation('lang_chinese'), variable=self.current_lang,
                                        value='zh_CN', command=self.switch_language)
        self.lang_menu.add_radiobutton(label=self.get_translation('lang_english'), variable=self.current_lang,
                                        value='en_US', command=self.switch_language)
        # Add the cascade itself, label will be updated in switch_language
        self.menubar.add_cascade(label=self.get_translation('menu_language'), menu=self.lang_menu, underline=0)

        # Theme Menu - Items added here, cascade label set initially
        self.theme_menu = Menu(self.menubar, tearoff=0)
        for theme_name in self.themes:
            self.theme_menu.add_radiobutton(label=theme_name, variable=self.selected_theme,
                                      command=self.apply_theme)
        # Add the cascade itself, label will be updated in switch_language
        self.menubar.add_cascade(label=self.get_translation('menu_theme'), menu=self.theme_menu, underline=0)

    # ========================================================================
    #                         FIXED switch_language
    # ========================================================================
    def switch_language(self):
        """Updates the UI elements to the selected language."""
        lang = self.current_lang.get()
        print(f"Switching language to: {lang}") # Keep for debugging

        # 1. Update Window Title
        self.root.title(self.get_translation('app_title'))

        # 2. Update Menu Labels using positional indices
        try:
            # Update top-level menu cascade labels
            # Index 0 = Language Menu Cascade, Index 1 = Theme Menu Cascade
            self.menubar.entryconfig(0, label=self.get_translation('menu_language'))
            self.menubar.entryconfig(1, label=self.get_translation('menu_theme'))

            # Update language options within the language menu itself
            # Index 0 = Chinese Radiobutton, Index 1 = English Radiobutton
            ...
            self.lang_menu.entryconfig(1, label=self.get_translation('lang_english'))
             ...
             # Log error if menu indices are somehow incorrect
             print(f"ERROR configuring menu items during language switch: {e}")
        except IndexError as e:
             print(f"ERROR: Menu index out of bounds during language switch: {e}")


        # 3. Update Widget Texts using stored references (assuming they exist)
        try:
    ...
    def create_widgets(self):
        ...
        self.watermark_check.grid(row=4, column=0, padx=5, pady=5, sticky=tk.W)
        self.watermark_entry = ttk.Entry(self.input_frame, textvariable=self.watermark_path, width=50, state=tk.DISABLED)
        self.watermark_entry.grid(row=4, column=1, padx=5, pady=5, sticky=tk.EW)
        ...
        self.watermark_browse_btn.grid(row=4, column=2, padx=5, pady=5)

        self.video_encoder_label = ttk.Label(self.input_frame, text=self.get_translation('video_encoder_label'))
        self.video_encoder_label.grid(row=5, column=0, padx=5, pady=5, sticky=tk.W)
        self.video_encoder_combo = ttk.Combobox(self.input_frame, textvariable=self.video_encoder,
                                          ...
                                          state="readonly")
        self.video_encoder_combo.grid(row=5, column=1, columnspan=2, padx=5, pady=5, sticky=tk.EW)

        self.audio_handling_label = ttk.Label(self.input_frame, text=self.get_translation('audio_handling_label'))
        self.audio_handling_label.grid(row=6, column=0, padx=5, pady=5, sticky=tk.W)
        self.audio_handling_combo = ttk.Combobox(self.input_frame, textvariable=self.audio_handling,
                                            ...
                                            state="readonly")
        self.audio_handling_combo.grid(row=6, column=1, columnspan=2, padx=5, pady=5, sticky=tk.EW)
        ...
        self.log_area = scrolledtext.ScrolledText(self.log_frame, wrap=tk.WORD, height=15, state=tk.DISABLED)
        self.log_area.pack(fill=tk.BOTH, expand=True)
        # Apply initial theme colors to log area right after creation - Moved to end of apply_theme


    # --- Helper Functions (log, browse*, toggle_watermark_entry - unchanged) ---
